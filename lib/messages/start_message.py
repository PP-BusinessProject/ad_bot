...
class StartMessage(object):
    async def start_message(
        ...
        if isinstance(chat_id, InputModel):
            ...
            if input is not None:
                await self.awaiting_input(
                    *(input, message_id),
                    data=Query(self.AWAITING_INPUT.CANCEL),
                    query_id=query_id,
                )
            ...
            *(chat_id, message_id),
            text='\n'.join(
                ...
                for _ in (
                    ...
                    + user.created_at.strftime(r'%Y-%m-%d %H:%M:%S')
    ...
    def user_create_listeners(self: 'AdBotClient', /) -> None:
        ...
        def _after_insert(_: Any, __: Any, user: UserModel, /) -> None:
            self.notify_subscription_end_job_init(
                user, self.NOTIFY_SUBSCRIPTION_END
            )
