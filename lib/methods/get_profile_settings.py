...
class GetProfileSettings(object):
    async def get_profile_settings(
        ...
        *,
        ...
        """
        user_peer = await self.resolve_peer(user_id or bot.owner.id)
        user_full: UserFull = await self.send(GetFullUser(id=user_peer))
        ...
        avatars: list[InputMediaPhoto] = [
            ...
            async for photo in self.iter_profile_photos(
                user_id or bot.owner.id
            )
        ]
        if not avatars:
