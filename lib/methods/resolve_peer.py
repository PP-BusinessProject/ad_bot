        ...
        if isinstance(peer_id, int):
            try:
                ...
                if peer_type == 'user':
                    ...
                    await self.fetch_peers(
                        await self.send(GetUsers(id=[user]))
                    )
                elif peer_type == 'chat':
                    await self.send(GetChats(id=[-peer_id]))
                else:
                    ...
                    try:
                        await self.send(GetChannels(id=[channel]))
            ...
            try:
                ...
                try:
                    ...
                    r = await self.send(ResolveUsername(username=peer_id))
                    try:
                        return await self.storage.get_peer_by_username(peer_id)
                    except KeyError as e:
                        raise PeerIdInvalid from e
